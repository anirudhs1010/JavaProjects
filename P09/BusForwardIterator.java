 ...
 * An iterator that does an <b>in-order traversal</b> of the BST starting at the
 given node and
 * iterates forward until we get to the latest bus. In other words, we iterate over
 all Bus-es in
 * sorted order.
 */
public class BusForwardIterator implements Iterator<Bus> {
  /**
   * Use a stack to keep track of where we are in the iteration. The top of the
   stack is the next
   * node we need to return.
   */
  private Stack<Node<Bus>> stack;
  /**
   * Construct a new BusForwardIterator that iterates over all nodes in the tree
   with root `root`
   * starting from the node `node`.
   *
   * @param root the root of the BST we are iterating.
   * @param node the starting point for the iteration. In other words, this is the
  first node
   * returned. The iterator will return this node and every node after
  it, in sorted
   * order.
   */
...
// Go to parent... nothing to do.
        ...
        while (!stack.isEmpty() &&
                stack.peek().getValue().compareTo(toReturn.getValue()) <= 0) {
          stack.pop(); // Go up
        }
      }
    }
